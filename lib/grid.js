...
exports.renameColumn = exports.nameCellMap = exports.AGGREGATE_COL_NAME = exports.ROW_NUMBER_COL_NAME = exports.NAMED_CELL_PREFIX = void 0;
exports.recalculateAllCells = recalculateAllCells;
exports.validateName = validateName;
exports.createGrid = createGrid;
exports.addColumn = addColumn;
exports.addRow = addRow;
exports.createAddColButton = createAddColButton;
exports.createAddRowButton = createAddRowButton;
exports.createAddRowAfterButton = createAddRowAfterButton;
exports.createAddRowBeforeButton = createAddRowBeforeButton;
exports.createDeleteRowButton = createDeleteRowButton;
exports.createCopyRowsButton = createCopyRowsButton;
...
const cellEditor_1 = __importDefault(require("./cellEditor"));
const formulacell_1 = require("./formulacell");
const formula_1 = require("./formula");
const assert_1 = require("assert");
const Cell_1 = require("./Cell");
const dependencyGraphUtils_1 = require("./dependencyGraphUtils");
const cellRenderer_1 = __importDefault(require("./cellRenderer"));
...
(function (Position) {
    ...
    Position[Position["NoPos"] = 2] = "NoPos";
})(Position || (Position = {}));
var PasteType;
(function (PasteType) {
    PasteType[PasteType["Cut"] = 0] = "Cut";
    PasteType[PasteType["Copy"] = 1] = "Copy";
})(PasteType || (PasteType = {}));
function recalculate(event, model) {
    recalculateAllCells(event.api, event.columnApi, model);
}
let col_num = 1;
exports.NAMED_CELL_PREFIX = "#namedcell:";
exports.ROW_NUMBER_COL_NAME = "#";
exports.AGGREGATE_COL_NAME = "Aggregation";
...
exports.nameCellMap = new Map();
function recalculateAllCells(gridApi, columnApi, model) {
    ...
    (0, dependencyGraphUtils_1.populatePrecedentsOfAllCells)(gridApi, columnApi, envVar);
    // Construct dependency graph using precedents of the formula cells
    let depGraph = (0, dependencyGraphUtils_1.constructDependencyGraph)(gridApi);
    // Check whether a cycle exists in the dependency graph
    let depGraphOrderingValue = (0, dependencyGraphUtils_1.checkCycleInDepGraph)(gridApi, depGraph);
    ...
    if (cycles.length === 0) {
        ...
        for (const cellKey of recalculationOrder) {
            if (typeof cellKey === "string" && cellKey.startsWith(exports.NAMED_CELL_PREFIX)) {
                const cellValue = depGraph.getNodeData(cellKey.toString());
                if (cellValue instanceof Cell_1.Cell && cellValue.formulaCell) {
                    let ctx = new formula_1.EvaluationContext(gridApi, columnApi, cellValue.getRowNodeId(), envVar, false);
                ...
                if (rowNode && rowNode.id) {
                    let ctx = new formula_1.EvaluationContext(gridApi, columnApi, rowNode.id, envVar, false);
    ...
    let modelValue = []; // list of all the row data of the table
    ...
    allRowNodes.forEach(rowNode => {
        ...
        let rowValue = {};
        (0, assert_1.strict)(rowNode.id != undefined);
        if (rowNode.id != undefined) {
            let ctx = new formula_1.EvaluationContext(gridApi, columnApi, rowNode.id, envVar, false);
            for (const idx in columnDefs) {
                const key = columnDefs[idx]['field'];
                if (key === exports.ROW_NUMBER_COL_NAME || !rowData[key]) {
                    continue;
                }
                let nodeKey = key + "+" + rowNode.rowIndex;
...
function validateName(name, id, columnApi, cellMap) {
    ...
    let columnHeaderList = columnApi === null || columnApi === void 0 ? void 0 : columnApi.getColumns();
...
function onHeaderCellClicked(event) {
    ...
    if (cellEditingContext.formulaMode && cellEditingContext.cellEditorInstance) {
        const headerColumnClicked = event.target.outerText;
 ...
 */
const renameColumn = (gridOptions, colId, oldColumnName, newColumnName, element, model) => {
    var _a;
    ...
    let rowValues = [];
    const allRowNodes = (0, gridUtils_1.getAllRowNodes)(gridOptions.api);
    ...
    const newColumnDefs = (_a = gridOptions.api.getColumnDefs()) === null || _a === void 0 ? void 0 : _a.map((col) => {
        if (col.colId === colId) {
            newColumnDef = col;
...
const cellValueSetter = (params, model) => {
    var _a, _b, _c, _d;
    if (((_a = params.node) === null || _a === void 0 ? void 0 : _a.id) === undefined) {
        return true;
    }
    const columnName = params.colDef.field;
    // If the oldValue is null, which implies the cell doesn't exist yet; create a new cell
    if (!params.oldValue && columnName) {
        params.data[columnName] = new Cell_1.Cell(params.newValue);
    }
    ...
    if (params.newValue && params.newValue.formulaCell && columnName) {
        ...
        params.data[columnName].formulaCell = new formulacell_1.FormulaCell(formulaCellObj.formula.str, params.api, formulaCellObj.formula.value, formulaCellObj.formula.precedents);
    }
    // Update precedents of all the cells
    (0, dependencyGraphUtils_1.populatePrecedentsOfAllCells)(params.api, params.columnApi, envVar);
    // Construct dependency graph
    let depGraph = (0, dependencyGraphUtils_1.constructDependencyGraph)(params.api);
    const depGraphOrderingValue = (0, dependencyGraphUtils_1.checkCycleInDepGraph)(params.api, depGraph);
    const cycles = depGraphOrderingValue.cycles;
    // If cycle exists, need to call recalculateCells to populate error
    // in all the cells involved in the cycle
    // const columnName = params.colDef.field;
    let nodeKey = columnName + "+" + ((_b = params.node) === null || _b === void 0 ? void 0 : _b.rowIndex);
    if ((_c = params.node) === null || _c === void 0 ? void 0 : _c.isRowPinned()) {
        nodeKey = columnName + "+" + ((_d = params.node) === null || _d === void 0 ? void 0 : _d.id);
    }
    if (cycles.length > 0) {
        let isNodeInvolvedInCycle = false;
        for (const cycle of cycles) {
            isNodeInvolvedInCycle || (isNodeInvolvedInCycle = cycle.includes(nodeKey));
...
const cellValueGetter = (params) => {
    var _a;
    const colName = params.colDef.field;
    if (colName === exports.ROW_NUMBER_COL_NAME) {
        return (_a = params.node) === null || _a === void 0 ? void 0 : _a.rowIndex;
    }
    if (!colName) {
        return "";
    }
...
function createGrid(props) {
    const [modelState, updateModelState] = (0, widget_model_1.useModelState)("value");
    envVar = (0, widget_model_1.useModelState)("env_variables");
    ...
    const inputRows = props.initialData;
    ...
    for (let row of inputRows) {
        var rowWithCells = new Object();
        // Value is assigned by the cell-renderer automatically
        rowWithCells[exports.ROW_NUMBER_COL_NAME] = null;
        ...
        for (let cell of Object.entries(row)) {
            let col_name = cell[0];
            let cell_val = cell[1];
            let cellValue = new Cell_1.Cell({ column: col_name });
            if (typeof cell_val == "string" && cell_val.startsWith("=")) {
                let formulaCell = new formulacell_1.FormulaCell(cell_val.substr(1), null);
    ...
    model = props.model;
    const [rowData, setRowData] = (0, react_1.useState)(rowsWithCells);
    const [gridApi, setGridApi] = (0, react_1.useState)(null);
    const [columnApi, setColumnApi] = (0, react_1.useState)(null);
    // Formula text inside the formula bar
    const [formulaBarText, setFormulaBarText] = (0, react_1.useState)("");
    // Name text inside of the name bar
    const [nameBarText, setNameBarText] = (0, react_1.useState)("");
    const onGridReady = (event) => {
        ...
        if (event.api != null) {
            (_a = event.api) === null || _a === void 0 ? void 0 : _a.forEachNode(rowNode => {
                for (const key in rowNode.data) {
                    ...
                    if (cell && cell.formulaCell != null) {
                        ...
                        cell.formulaCell.evaluate(new formula_1.EvaluationContext(event.api, event.columnApi, rowNode.id, envVar, false));
                    }
                }
            });
        }
        ...
        const allRowNodes = (0, gridUtils_1.getAllRowNodes)(event.api);
    ...
    const onCellEditingStarted = (event) => {
        ...
        const cellText = (_a = event.value) === null || _a === void 0 ? void 0 : _a.formula;
        if (cellText) {
            cellEditingContext.formulaMode = true;
        }
        else {
    ...
    const onCellFocused = (e) => {
        ...
        if (cellVal.formulaCell) {
            let ctx = new formulaStringVisitor_1.FormulaStringContext(e.api);
        ...
        let isNameValid = validateName(cellVal.name, id, e.columnApi, exports.nameCellMap);
        setNameIsValid({
    ...
    const processCellFromClipboard = (params) => {
        ...
        if (colName && params.node) {
            ...
            const cell = new Cell_1.Cell(nodeData);
    ...
    const onPasteEnd = (params) => {
        ...
        if (focusedCell) {
            ...
            const cellValue = rowNode === null || rowNode === void 0 ? void 0 : rowNode.data[colName];
            if (cellValue) {
    ...
    let columnList = columns.map((column, index) => {
        let columnProperties = {
            ...
            valueSetter: (params) => cellValueSetter(params, props.model),
            valueGetter: (params) => cellValueGetter(params),
            context: context,
        ...
        if (column === exports.ROW_NUMBER_COL_NAME) {
        ...
        if (column === exports.AGGREGATE_COL_NAME) {
            columnProperties["headerName"] = "";
            columnProperties["lockPosition"] = "right";
        }
        return columnProperties;
    });
    const getMainMenuItems = (0, react_1.useCallback)((params) => {
        // For first column (Row_number), only allow 'Add Column Right' action
        if (params.column.colId === exports.ROW_NUMBER_COL_NAME) {
            return [
                {
        ...
        if (params.column.colId === exports.AGGREGATE_COL_NAME) {
            return [
                {
        ...
        return [
            {
                ...
                action: () => {
                    ...
                    (0, exports.renameColumn)(props.gridOptions, oldColumnName, oldColumnName, newColumnName, null, props.model);
                },
            },
            {
                name: "Add Column Right",
    ...
    const fillOperation = (fillOperationParams) => {
        var _a, _b, _c, _d;
        const direction = fillOperationParams.direction;
        const currentIndex = fillOperationParams.currentIndex; // Array index of currently processed value from array of all values to be processed
        const curRowIndex = fillOperationParams.rowNode.rowIndex; // Row index of currently processed value
        if (curRowIndex === null) {
            return;
        }
        ...
        const selectedRange = fillOperationParams.api.getCellRanges(); // Get the selected range being used for fill-handle
        if (!selectedRange || selectedRange.length === 0) {
            return;
        }
        const startRowIndex = (_a = selectedRange[0].startRow) === null || _a === void 0 ? void 0 : _a.rowIndex;
        ...
        const endRowIndex = (_b = selectedRange[0].endRow) === null || _b === void 0 ? void 0 : _b.rowIndex;
        ...
        let areAllValuesNums = null;
        if (direction === "up" || direction === "down") {
            areAllValuesNums = (0, gridUtils_1.areAllValuesNumbersWithContDiffInVertRange)(fillOperationParams.api, selectedRange, fillOperationParams.column.getColId());
        }
        else if (direction === "left" || direction === "right") {
            areAllValuesNums = (0, gridUtils_1.areAllValuesNumbersWithConstDiffInHorizRange)(fillOperationParams.api, selectedRange, curRowIndex);
        }
        if (areAllValuesNums && Array.isArray(areAllValuesNums) && areAllValuesNums[0] === true) {
            ...
            if (constDiff !== null) {
                ...
                if (direction === "down") {
                    let cellValue = endRowNode === null || endRowNode === void 0 ? void 0 : endRowNode.data[colName].value;
                    if (startRowIndex > endRowIndex) {
                        cellValue = startRowNode === null || startRowNode === void 0 ? void 0 : startRowNode.data[colName].value;
                    }
                    ...
                    let cellValue = startRowNode === null || startRowNode === void 0 ? void 0 : startRowNode.data[colName].value;
                    if (startRowIndex > endRowIndex) {
                        cellValue = endRowNode === null || endRowNode === void 0 ? void 0 : endRowNode.data[colName].value;
                    }
                    curCellValue.formulaCell = null;
                    curCellValue.value = cellValue + (-constDiff) * (currentIndex + 1);
                    return curCellValue;
                    ...
                    const lastSelectedCellValue = curRowNode === null || curRowNode === void 0 ? void 0 : curRowNode.data[lastSelectedColName].value;
                    const curCellValue = curRowNode === null || curRowNode === void 0 ? void 0 : curRowNode.data[colName];
                    curCellValue.formulaCell = null;
                    ...
                    const firstSelectedCellValue = curRowNode === null || curRowNode === void 0 ? void 0 : curRowNode.data[firstSelectedColName].value;
                    const curCellValue = curRowNode === null || curRowNode === void 0 ? void 0 : curRowNode.data[colName];
                    curCellValue.formulaCell = null;
        ...
        if (direction === "up" || direction === "down") {
            ...
            let startRowIndex = (_c = selectedRange[0].startRow) === null || _c === void 0 ? void 0 : _c.rowIndex;
            let endRowIndex = (_d = selectedRange[0].endRow) === null || _d === void 0 ? void 0 : _d.rowIndex;
            ...
            const correspondingCellValue = correspondingRowNode === null || correspondingRowNode === void 0 ? void 0 : correspondingRowNode.data[colName];
            let curCellValue = fillOperationParams.rowNode.data[colName];
    ...
    const createFooterData = () => {
        ...
        columns.forEach(column => {
            footerData[column] = new Cell_1.Cell({ column: column, value: 0 });
        });
        row.push(footerData);
        return row;
    };
    ...
    props.gridOptions.components = {
        agCellEditor: cellEditor_1.default,
    };
    ...
    const [nameIsValid, setNameIsValid] = (0, react_1.useState)({ hasError: false, message: "" });
    ...
    const handleFormulaBarChange = (event) => {
        ...
        const cellValue = rowNode === null || rowNode === void 0 ? void 0 : rowNode.data[column];
        //ge the new formula from the formulaBar
        let formula_new = event.target.value;
        if (cellValue.formulaCell == null) {
            cellValue.formulaCell = new formulacell_1.FormulaCell(formula_new, gridApi_);
        }
        ...
        if (rowNode && typeof rowNode.id === 'string') { // Check if rowNode.id is a string
            const evaluationContext = new formula_1.EvaluationContext(gridApi, columnApi, rowNode.id, envVar, false);
    ...
    const updateNameBarText = (event) => {
        ...
        let isNameValid = validateName(cell_name, id, columnApi, exports.nameCellMap);
        setNameIsValid({
            hasError: isNameValid["hasError"],
            message: isNameValid["message"],
        });
        ...
        if (!isNameValid["hasError"]) {
            ...
            exports.nameCellMap.set(id, cell_name);
        }
    ...
    let grid = (react_1.default.createElement("div", { style: { height: 1080, width: "100%" } },
        react_1.default.createElement("div", { style: { height: "0.8rem" } }),
        react_1.default.createElement("div", { style: { width: "30%" } },
            react_1.default.createElement("div", { style: barStyle },
                react_1.default.createElement("label", { htmlFor: "formulaBar", style: { marginRight: "1.2rem", verticalAlign: "middle" } }, "Formula Bar"),
                react_1.default.createElement("input", { id: "formulaBar", placeholder: "Insert Formula Here", value: formulaBarText, onKeyDown: (e) => onEnterKeyPressed(e), onChange: (e) => handleFormulaBarChange(e), style: Object.assign({ height: "1.1rem", backgroundColor: "#F0F0F0", verticalAlign: "middle", color: "#000000" }, spanStyle) })),
            react_1.default.createElement("div", { style: barStyle },
                react_1.default.createElement("label", { htmlFor: "nameBar", style: textStyle }, "Name Bar"),
                react_1.default.createElement("input", { id: "nameBar", placeholder: "Insert Name Here", value: nameBarText, onKeyDown: (e) => onEnterKeyPressed(e), onChange: (e) => updateNameBarText(e), style: Object.assign(Object.assign({ height: "1.1rem", backgroundColor: nameBarText ? "#FFFFFF" : "#F0F0F0", verticalAlign: "middle", color: nameBarText ? "#000000" : "#808080", borderColor: nameIsValid["hasError"] ? "#FF0000" : "#000000" }, spanStyle), (!nameIsValid["hasError"] ? {} : nameBarInvalidStyle)) }),
                nameIsValid["hasError"] && (react_1.default.createElement("span", { style: Object.assign(Object.assign({}, spanStyle), { marginLeft: "0.5rem" }) }, nameIsValid["message"])))),
        react_1.default.createElement("div", { className: "ag-theme-alpine", style: {
                height: 600,
                maxHeight: 1000,
                width: "100%",
                marginBottom: "auto",
            ...
            react_1.default.createElement(ag_grid_react_1.AgGridReact, { maintainColumnOrder: true, gridOptions: props.gridOptions, onCellFocused: onCellFocused, onRowDragEnd: handleRowDragEnd }))));
    cellEditingContext.gridOptions = props.gridOptions;
    return grid;
}
function addColumn(props, id = "None", pos) {
    ...
    if (cols.some((e) => e["field"] == id)) {
        ...
        cols === null || cols === void 0 ? void 0 : cols.push(new_col);
    }
    ...
    const allRowNodes = (0, gridUtils_1.getAllRowNodes)(props.gridOptions.api);
    allRowNodes.forEach(node => {
        let cellValue = new Cell_1.Cell({ column: newName, rowNodeId: node.id });
        cellValue.setValue(0);
        node.setDataValue(new_col.field, cellValue);
    });
    // Add onClick event listener to newly added column
    const columnSelector = HEADER_CLASS + `[col-id="${newName}"]`;
    const headerNode = document.querySelector(columnSelector);
    headerNode === null || headerNode === void 0 ? void 0 : headerNode.addEventListener("click", (event) => {
        onHeaderCellClicked(event);
    });
    recalculate(props.gridOptions, props.model);
}
function addRow(props, pos = "None") {
    var _a, _b, _c, _d;
    ...
    cols.forEach((column) => {
        ...
        let cellValue = new Cell_1.Cell({ column: columnName });
        cellValue.setValue(0);
        initialRowValue[columnName] = cellValue;
    });
    let newRowNodeId = null;
    let txnRes = null;
    if (props.gridOptions.api.getSelectedRows().length == 0) {
        txnRes = (_a = props.gridOptions.api) === null || _a === void 0 ? void 0 : _a.applyTransaction({ add: [initialRowValue] });
    }
    else {
        let addIndex = props.gridOptions.api.getSelectedNodes()[0]["rowIndex"];
        if (pos == "After") {
            txnRes = (_b = props.gridOptions.api) === null || _b === void 0 ? void 0 : _b.applyTransaction({ add: [initialRowValue], addIndex: addIndex + 1 });
        }
        else if (pos == "Before") {
            txnRes = (_c = props.gridOptions.api) === null || _c === void 0 ? void 0 : _c.applyTransaction({ add: [initialRowValue], addIndex: addIndex });
        }
        else {
            txnRes = (_d = props.gridOptions.api) === null || _d === void 0 ? void 0 : _d.applyTransaction({ add: [initialRowValue] });
        }
    }
...
function createAddColButton(props) {
    let button = (react_1.default.createElement("button", { style: { height: "4rem", width: "6rem", fontSize: "0.9rem" }, onClick: () => addColumn(props, "None", Position.NoPos) }, "Add Column"));
    return button;
}
function createAddRowButton(props) {
    let button = (react_1.default.createElement("button", { style: { height: "4rem", width: "6rem", fontSize: "0.9rem" }, onClick: () => addRow(props) }, "Add Row"));
    return button;
}
function createAddRowAfterButton(props) {
    let button = (react_1.default.createElement("button", { style: { height: "4rem", width: "6rem", fontSize: "0.9rem" }, onClick: () => addRow(props, "After") }, "Add Row After"));
    return button;
}
function createAddRowBeforeButton(props) {
    let button = (react_1.default.createElement("button", { style: { height: "4rem", width: "6rem", fontSize: "0.9rem" }, onClick: () => addRow(props, "Before") }, "Add Row Before"));
    return button;
}
function createDeleteRowButton(props) {
    let button = (react_1.default.createElement("button", { style: { height: "4rem", width: "6rem", fontSize: "0.9rem" }, onClick: () => deleteRow(props) }, "Delete Row"));
    return button;
}
function createCopyRowsButton(props) {
    let button = (react_1.default.createElement("button", { style: { height: "4rem", width: "6rem", fontSize: "0.9rem" }, onClick: () => copyRows(props) }, "Copy Rows"));
    return button;
}
function deleteColumn(props, id = "None") {
    let cols = props.gridOptions.api.getColumnDefs();
    if (cols.some((e) => e["field"] == id)) {
        let idx = 0;
        for (let i = 0; i < cols.length; i++) {
...
function extractCols(gridApi, columnApi) {
